...
async function pullAllFromAPI(isRetry = false) {
  ...
  try {
    // Timeout 30 detik — cukup untuk GAS cold start (kalau backend='sql',
    // MySQL biasanya jauh lebih cepat dari ini, timeout tetap dipakai
    // sebagai jaga-jaga saja)
    const r = await fetch(apiBase() + '?fn=read', {
      mode: 'cors',
