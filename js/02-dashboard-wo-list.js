...
async function loadWOPageFast(page) {
  try {
    const r = await fetch(
      CFG.apiUrl + '?fn=read_wo_page&page=' + page + '&pageSize=' + WO_LIST_PAGE_SIZE,
...
function woListPagerHtml(totalItems, totalPages) {
  ...
  const p = ST.woPage;
  return `
    <div class="wo-pager" style="display:flex;align-items:center;justify-content:center;gap:12px;padding:16px 0">
      <button class="btn-ghost" ${p <= 1 ? 'disabled' : ''} onclick="goToWOPage(${p - 1})">‹ Previous</button>
      <span style="font-size:12px;color:var(--text3)">Halaman ${p} dari ${totalPages}</span>
      <button class="btn-ghost" ${p >= totalPages ? 'disabled' : ''} onclick="goToWOPage(${p + 1})">Next ›</button>
    </div>`;
}
...
function toggleView() {
  ...
  document.getElementById('viewToggleBtn').textContent = ST.viewMode === 'card' ? '☰ Table View' : '⊞ Card View';
  renderWOList();
}
