...
async function refreshSessionToken() {
  ...
  try {
    const resp = await fetch(
      CFG.apiUrl + '?fn=login&username=' + encodeURIComponent(cachedUser.username) + '&password=' + encodeURIComponent(cachedUser.password),
  ...
  try {
    for (const key of Object.keys(batch)) {
      ...
      try {
          ...
          const result = await _fetchSilent(CFG.apiUrl, { fn: 'upsert', sheet: 'WO_ORDERS', row: item.data });
          reconcileWOIdIfRenamed(item.data.id, result && result.id);
        } else if (item.type === 'delete_wo') {
          await _fetchSilent(CFG.apiUrl, { fn: 'delete', sheet: 'WO_ORDERS', id: item.id });
        } else if (item.type === 'upsert_tech') {
          await _fetchSilent(CFG.apiUrl, { fn: 'upsert', sheet: 'WO_TECHNICIANS', row: item.data });
        }
