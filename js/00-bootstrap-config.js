...
const CFG = {
  ...
  // ═════════════════ MIGRASI KE MYSQL — SAKLAR ONLINE/OFFLINE ═════════════════
  // 'sheets' = pakai backend lama (Google Apps Script + Sheets) — DEFAULT,
  //            tidak ada yang berubah sampai kamu sengaja ganti ini.
  // 'sql'    = pakai backend baru (PHP + MySQL, lihat folder sql-backend/).
  //
  // Alur migrasi yang disarankan:
  //   1. Tes dulu dengan sqlApiUrlLocal (MySQL lokal di laptop/PC, XAMPP/Laragon).
  //   2. Kalau sudah oke, upload sql-backend/ + schema.sql ke web hosting,
  //      lalu isi sqlApiUrlOnline dengan URL index.php di hosting itu.
  //   3. Ganti `backend` jadi 'sql' → otomatis semua fetch pakai MySQL.
  //      GAS/Sheets ('offline') tidak dipanggil sama sekali lagi setelah ini —
  //      itulah cara "mematikan offline" yang dimaksud: cukup ganti nilai ini.
  backend:         'sheets',
  sqlApiUrlLocal:  'http://localhost/sql-backend/index.php',
  sqlApiUrlOnline: 'https://domainmu.com/sql-backend/index.php',
};

// Pilih URL SQL sesuai environment (local dev vs sudah online di hosting).
// Deteksi otomatis: localhost/127.0.0.1 → pakai sqlApiUrlLocal, selain itu →
// sqlApiUrlOnline. Bisa dipaksa manual dengan set CFG.sqlApiUrl langsung.
CFG.sqlApiUrl = CFG.sqlApiUrl || (
  ['localhost', '127.0.0.1'].includes(location.hostname) ? CFG.sqlApiUrlLocal : CFG.sqlApiUrlOnline
);

// Dipakai di SEMUA tempat yang sebelumnya langsung pakai CFG.apiUrl untuk
// baca/tulis WO, users, login (bukan untuk equipDbApi/maintwareApiUrl —
// dua itu tetap ke sistem terpisah, tidak termasuk migrasi ini).
function apiBase() {
  return CFG.backend === 'sql' ? CFG.sqlApiUrl : CFG.apiUrl;
}
...
function fetchUsersFromServer(timeoutMs) {
  ...
  return fetch(apiBase() + '?fn=get_users', {
    mode: 'cors',
